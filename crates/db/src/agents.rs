...
/// Filter inputs for `search()`. All optional; NULL placeholders skip the
/// corresponding predicate.
///
/// `has_endpoint` is a Phase-6 column on `agent_endpoints` (lands with the
/// endpoint-prober worker on `feat/phase-4-w3-endpoint-prober`); we proxy it
/// through `agent_wallet IS NOT NULL` for now so the API DTO is stable. The
/// proxy is documented in the handler so callers know the semantics.
#[derive(Debug, Clone, Default)]
...
/// Non-paginated search used by `POST /api/v1/agents/search`. Hard-caps at
/// 200 rows; clients that need more should switch to the paginated `list()`.
