...
fn req_with_secret(secret: &str, query: Option<&str>) -> Request {
    let uri = match query {
...
async fn serve_writes_worker_runs_and_skips_on_dryrun() {
    ...
    // ---------------- advisory lock: contention returns skipped ----------
    // Plan §3 invariant #1: two concurrent invocations of the same worker
    // must NOT both run the body. Spawn one body that parks for 250 ms,
    // race it against a second invocation, and assert exactly one audit
    // row landed and the second response carries `skipped: already_running`.
    let before = count_runs(&pool).await;
    ...
    let slow_handle = {
        ...
        tokio::spawn(async move {
            serve_with_context(WORKER_NAME, req_with_secret("right", None), move |_ctx| {
                let lock_held = lock_held.clone();
                let lock_released = lock_released.clone();
                async move {
                    // Signal that we've grabbed the lock and are inside the body.
                    lock_held.notify_one();
                    // Hold the lock until the second invocation has tried + failed.
    ...
    // Wait until the slow invocation is INSIDE the lock; then race a contender.
    lock_held.notified().await;
    let contender = serve_with_context(WORKER_NAME, req_with_secret("right", None), |_ctx| async {
        panic!("contender body must NOT run while lock is held");
    })
    .await;
    assert_eq!(contender.status(), StatusCode::OK);
    let contender_text = match contender.into_body() {
        Body::Text(t) => t,
        other => panic!("expected JSON Body::Text, got {other:?}"),
    };
    let contender_json: serde_json::Value = serde_json::from_str(&contender_text).expect("contender json");
    assert_eq!(contender_json["skipped"], serde_json::Value::Bool(true));
    assert_eq!(
        contender_json["reason"],
        serde_json::Value::String("already_running".into())
    );

    // Let the slow body finish; assert exactly one audit row was written.
    lock_released.notify_one();
    let slow_resp = slow_handle.await.expect("slow join");
    assert_eq!(slow_resp.status(), StatusCode::OK);
    let after = count_runs(&pool).await;
    assert_eq!(
        after,
        before + 1,
        "contention must yield exactly one worker_runs row, not two"
    );
