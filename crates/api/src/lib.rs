...
//!
//! Mounted by `api/v1/index.rs` (Vercel function entrypoint) and by
//! `crates/dev-server` (local). Both share the exact same `router()` so prod
//! and local exercise identical handler code.
//!
